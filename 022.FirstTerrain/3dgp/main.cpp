...
GLuint idTexNone;		// solid colour texture
...
void render()
{
	...
	Program.SendUniform("lightPoint1.diffuse", 0.6, 0.6, 0.6); //brightness													   
	Program.SendUniform("lightPoint1.specular", 0.5, 0.5, 0.5); //brightestness .0 to 1.0													   
	//Program.SendUniform("lightPoint1.att_quadratic", 5.0f);
	Program.SendUniform("lightPoint1.position", 6.0, 0.0, -3.0);
	Program.SendUniform("fogColor", 0.867, 0.867, 0.867);
	Program.SendUniform("fogDensity", 0.1);

	// setup materials for the terrain
	//Program.SendUniform("materialDiffuse", 0.6f, 0.9f, 0.6f);	// grassy green
	...
	m = translate(m, vec3(0.5f, 3.35f, -4.0f));
	...
	m = translate(matrixView, vec3(0, Y, 0));
	m = translate(m, vec3(6.5f, 5.8f, -4.0f));
	m = scale(m, vec3(0.24f, 0.24f, 0.24f));
